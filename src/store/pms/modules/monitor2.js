...
const monitor2 = {
    ...
    actions: {
        handleGraphData({ rootState, state, commit }) {
            ...
            // };
            axios
                .get(`${rootState.globalIP}/api/v1/pumpBoard/disCharge`)
                .then((data) => {
                    ...
                    for (let i = 0; i < datas.length; i++) {
                        let obj = datas[i];
                        let idName = obj.pump_board_temp_id;
                        if (idName === state.pump_board_temp_id[0].id)
                            dataArr_1.push(obj);
                        if (idName === state.pump_board_temp_id[1].id)
                            dataArr_2.push(obj);
                        if (idName === state.pump_board_temp_id[2].id)
                            dataArr_3.push(obj);
                        if (idName === state.pump_board_temp_id[3].id)
                            dataArr_4.push(obj);
                    }
            ...
            //알람
            axios
                .get(`${rootState.globalIP}/api/v1/pumpBoard/alarm`)
                .then((data) => {
                    const datas = data.data.datas;
                    if (datas.length !== 0) {
        ...
        handleDatePicker({ rootState, state, commit }) {
            ...
            axios
                .post(
                    `${rootState.globalIP}/api/v1/pumpBoard/detailInfo`,
                    params
                )
                .then((data) => {
        ...
        getPumpInfo({ rootState, commit }) {
            axios
                .get(`${rootState.globalIP}/api/v1/pumpBoard/info`)
