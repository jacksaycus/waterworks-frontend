...
const monitor4 = {
    ...
    mutations: {
        ...
        handleDatePickGraph(state, datas) {
            ...
            state.detailData.pac_ch_single_i = [];
            let current = [];
            let diff_voltage = [];
            let pac_ch_single_i = [];
            ...
            for (let i = 0; i < datas.length; i++) {
                ...
                pac_ch_single_i.push([
                    date.getTime(),
                    datas[i].pac_ch_single_i,
                ]);
            }
            state.detailData.current = current;
            state.detailData.diff_voltage = diff_voltage;
            state.detailData.pac_ch_single_i = pac_ch_single_i;
        },
    ...
    actions: {
        handleGraphData({ rootState, state, commit, dispatch }) {
            ...
            axios
                .post(
                    `${rootState.globalIP}/api/v1/pacTube/currentInfo`,
                    params
                )
                .then((data) => {
                    ...
                    for (let i = 0; i < datas.length; i++) {
                        let obj = datas[i];
                        let idName = obj.pac_tube_pump_id;
            ...
            //알람
            axios
                .get(`${rootState.globalIP}/api/v1/pacTube/alarm`)
                .then((data) => {
                    const datas = data.data.datas;
                    if (datas.length !== 0) {
        ...
        handleDatePicker({ rootState, state, commit }) {
            ...
            axios
                .post(`${rootState.globalIP}/api/v1/pacTube/detailInfo`, params)
                .then((data) => {
        ...
        //세부페이지 전류 조회
        handleDatePicker2({ rootState, state, commit }) {
            const params = {
                endDate: state.pickDate.to,
                startDate: state.pickDate.from,
            };
            axios
                ...
                .then((data) => {
                    ...
                    commit('handleDatePickGraph2', dataArr_1);
                });
            // dispatch('pacTubeInfo');
        },

        //상태정보 조회
        pacTubeInfo({ rootState, commit }) {
            axios
                .get(`${rootState.globalIP}/api/v1/pacTube/pacTubeInfo`)
                .then((data) => commit('pacTubeInfo', data.data.datas));
        },
