<template>
    <div>
        ...
        <Frame />
         <div class="chart-box">
             <div class="chart" v-for="(value, index) in store.state.process"  :key="index"  @click="selectBarchart(index)" >
             <div class="selectbox" :class="{chartSelected: value.selected}"></div>
             <Bar :name="value.title" :value="value.normal" :value2="value.err"/>
         </div>
         </div>
    </div>
</template>
