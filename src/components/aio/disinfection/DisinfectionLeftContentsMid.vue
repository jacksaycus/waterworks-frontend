  ...
  <div class="main">
    ...
    <div class="contents-value">
      ...
      <div class="contents-value__row">
        ...
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri1_set_max" v-on:input="updateInput($event, 'g_peri1_set_max')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri1_set_max | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri1_set_min" v-on:input="updateInput($event, 'g_peri1_set_min')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri1_set_min | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri1_chg_limit_for_onetime" v-on:input="updateInput($event, 'g_peri1_chg_limit_for_onetime')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri1_chg_limit_for_onetime | numFormat('0.00') }}</span></div>
...
<div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri2_set_max" v-on:input="updateInput($event, 'g_peri2_set_max')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri2_set_max | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri2_set_min" v-on:input="updateInput($event, 'g_peri2_set_min')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri2_set_min | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri2_chg_limit_for_onetime" v-on:input="updateInput($event, 'g_peri2_chg_limit_for_onetime')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri2_chg_limit_for_onetime | numFormat('0.00') }}</span></div>
...
<script>
import { PUT_DISINFECTION_CONTROL_PERI } from '@/store/aio/modules/disinfection'
export default {
  data: () => ({
    // isModifyMode: false
  }),
  methods: {
    ...
    updateControl: function() {
      ...
      if (this.$store.state.disinfection.isModifyMode) {
        if (this.$store.state.disinfection.latestModify.g_peri1_set_max === ''
           || this.$store.state.disinfection.latestModify.g_peri1_set_min === ''
           || this.$store.state.disinfection.latestModify.g_peri1_chg_limit_for_onetime === ''
           || this.$store.state.disinfection.latestModify.d1_target_cl === ''
           || this.$store.state.disinfection.latestModify.e1_target_cl === ''
           || this.$store.state.disinfection.latestModify.g_peri2_set_max === ''
           || this.$store.state.disinfection.latestModify.g_peri2_set_min === ''
           || this.$store.state.disinfection.latestModify.g_peri2_chg_limit_for_onetime === ''
          ...
          let obj = {}
          obj.g_peri1_set_max = parseFloat(this.$store.state.disinfection.latestModify.g_peri1_set_max)
          obj.g_peri1_set_min = parseFloat(this.$store.state.disinfection.latestModify.g_peri1_set_min)
          obj.g_peri1_chg_limit_for_onetime = parseFloat(this.$store.state.disinfection.latestModify.g_peri1_chg_limit_for_onetime)
          ...
          obj.g_peri2_set_max = parseFloat(this.$store.state.disinfection.latestModify.g_peri2_set_max)
          obj.g_peri2_set_min = parseFloat(this.$store.state.disinfection.latestModify.g_peri2_set_min)
          obj.g_peri2_chg_limit_for_onetime = parseFloat(this.$store.state.disinfection.latestModify.g_peri2_chg_limit_for_onetime)
          ...
          if ((obj.g_peri1_set_max <= peri_min_limit || obj.g_peri1_set_max >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '1계열 주입률 상한 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
          if ((obj.g_peri1_set_min <= peri_min_limit || obj.g_peri1_set_min >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '1계열 주입률 하한 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
          if ((obj.g_peri1_chg_limit_for_onetime <= peri_min_limit || obj.g_peri1_chg_limit_for_onetime >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '1계열 1회 변경 주입률 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
          ...
          if ((obj.g_peri2_set_max <= peri_min_limit || obj.g_peri2_set_max >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '2계열 주입률 상한 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
          if ((obj.g_peri2_set_min <= peri_min_limit || obj.g_peri2_set_min >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '2계열 주입률 하한 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
          if ((obj.g_peri2_chg_limit_for_onetime <= peri_min_limit || obj.g_peri2_chg_limit_for_onetime >= peri_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '2계열 1회 변경 주입률 설정 범위', text2: peri_min_limit + ' ~ ' + peri_max_limit})
            return
          }
  ...
  watch: {
    '$store.state.disinfection.latestModify.g_peri1_set_max': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri1_set_max'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri1_set_max'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri1_set_max'] = oldVal
        }
      }
    },
    '$store.state.disinfection.latestModify.g_peri1_set_min': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri1_set_min'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri1_set_min'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri1_set_min'] = oldVal
        }
      }
    },
    '$store.state.disinfection.latestModify.g_peri1_chg_limit_for_onetime': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri1_chg_limit_for_onetime'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri1_chg_limit_for_onetime'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri1_chg_limit_for_onetime'] = oldVal
        }
      }
    },
    ...
    '$store.state.disinfection.latestModify.g_peri2_set_max': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri2_set_max'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri2_set_max'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri2_set_max'] = oldVal
        }
      }
    },
    '$store.state.disinfection.latestModify.g_peri2_set_min': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri2_set_min'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri2_set_min'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri2_set_min'] = oldVal
        }
      }
    },
    '$store.state.disinfection.latestModify.g_peri2_chg_limit_for_onetime': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.disinfection.latestModify['g_peri2_chg_limit_for_onetime'] = newVal
      } else {
        if (this.checkNumberFormat(newVal)) {
          this.$store.state.disinfection.latestModify['g_peri2_chg_limit_for_onetime'] = newVal
        } else {
          this.$store.state.disinfection.latestModify['g_peri2_chg_limit_for_onetime'] = oldVal
        }
      }
    },
